...
- `/addressAutoComplete.php`:
  - In the `getMappedResultsBySource` function, add your API source to the `$allowedSources` variable.
  - The API response may vary depending on the service or endpoint used. In the `getApiAttributeIndex` function, add a mapping to your API source only if the attribute in the API response JSON that contains the list of addresses is not the first attribute.
...
If you cannot make the development yourself, but have found a useful API, please [open a Github Issue](https://github.com/Research-IT-Swiss-TPH/redcap-address-auto-complete/issues) describing the details about the API source to be added.
...
mrnf.gouv.qc.ca | Canada (Quebec only) | Unlimited      | no             |
